...
pars = {"input_data": sino_an_noisy, "radius": 2}  # input grayscale image

start_time = timeit.default_timer()
filtered = MEDIAN_FILT(pars["input_data"].copy(order="C"), pars["radius"])
...
start_time = timeit.default_timer()
dezingered = MEDIAN_DEZING(
    pars["input_data"].copy(order="C"), pars["radius"], pars["mu_threshold"]
)
txtstr = "%s = %.3fs" % ("elapsed time", timeit.default_timer() - start_time)
print(txtstr)
...
start_time = timeit.default_timer()
filtered_gpu = MEDIAN_FILT_GPU(pars["input_data"], pars["radius"])
txtstr = "%s = %.3fs" % ("elapsed time", timeit.default_timer() - start_time)
print(txtstr)
...
start_time = timeit.default_timer()
gpu_dezingered = MEDIAN_DEZING_GPU(
    pars["input_data"], pars["radius"], pars["mu_threshold"]
)
txtstr = "%s = %.3fs" % ("elapsed time", timeit.default_timer() - start_time)
print(txtstr)
