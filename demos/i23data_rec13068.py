...
h5py_list = h5py.File('../rawdata/13068/13068.nxs','r')
...
#%%
[RecFISTA_TV_os,upd_mask] = Rectools.FISTA(np.transpose(data_norm[det_y_crop,:,0]), \
                              weights=data_raw_norm,\
                              iterationsFISTA = 15, \
                              regularisation = 'NDF_MASK', \
...
plt.figure()
plt.imshow(RecFISTA_TV_os, vmin=-0.001, vmax=0.005, cmap="gray")
plt.title('Iterative FISTA-TV reconstruction')
plt.show()
#%%
from sklearn.mixture import GaussianMixture

inputdata = RecFISTA_TV_os.reshape(N_size**2, 1)/np.max(RecFISTA_TV_os)
hist, bin_edges = np.histogram(inputdata, bins=100)

classif = GaussianMixture(n_components=5,covariance_type="full")
