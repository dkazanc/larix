...
vert_tuple = [i for i in range(700,1100)] # selection of vertical slice

h5py_list = h5py.File('../rawdata/13062/13062.nxs','r')
...
det_y_crop = [i for i in range(52,detectHoriz)]
...
RecFISTA_TV_os = Rectools.FISTA(np.transpose(data_norm[det_y_crop,:,0]), \
                              ...
                              regularisation_parameter = 0.00000035,\
                              regularisation_iterations = 300,\
                              lipschitz_const = lc)

plt.figure()
plt.imshow(RecFISTA_TV_os, vmin=0, vmax=0.005, cmap="gray")
plt.show()
#%%
from tomorec.methodsIR import RecToolsIR
# set parameters and initiate a class object
...
RecFISTA_Huber_TV_os = Rectools.FISTA(np.transpose(data_norm[det_y_crop,:,0]), \
                              weights=data_raw_norm,\
                              iterationsFISTA = 20, \
                              #huber_data_threshold = 0.4,\
                              student_data_threshold = 0.93,\
                              regularisation = 'FGP_TV', \
...
plt.figure()
plt.imshow(RecFISTA_Huber_TV_os, vmin=0, vmax=0.005, cmap="gray")
plt.show()
#%%
