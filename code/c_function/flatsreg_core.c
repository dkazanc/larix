...
void flatsreg_main(unsigned short *flat, unsigned short *dark, unsigned short *proj, int x, int y, int x1, int y1, int drift_window, int dimX, int dimY, int flats_num, float *error_vec, float *proj_norm, float *proj_norm_corr)
{
/*printf("Hello \n");*/
...
size_p_x = (x1 - x); /* size of the selected patch */
size_p_y = (y1 - y);
...
for(z=0; z < flats_num; z++) {
  for(i_m=-drift_window; i_m<=drift_window; i_m++) {
   ...
   printf("%i %i %i \n", shiftX, shiftY, flatSel);
   
/* we know required shifts and the number of the selected flat field, lets normalise */
float norm_v;
#pragma omp parallel for shared(proj_norm_corr) private(i, j, norm_v)
for(i=0; i < dimX; i++) {
    for(j=0; j < dimY; j++) {
    /*
    norm_v = (float)(proj[j*dimX + i] - dark[j*dimX + i]) / (float)(flat[flatSel*dimX*dimY + j*dimX + i] - dark[j*dimX + i]);
    if (norm_v != 0.0f) proj_norm[j*dimX + i] = -logf(norm_v);
    else proj_norm[j*dimX + i] = 0.0f;
    */
    if (((i+shiftY) >= 0) && ((i+shiftY) < dimX)) {
        if (((j+shiftX) >= 0) && ((j+shiftX) < dimY)) {
        norm_v = (float)(proj[j*dimX + i] - dark[j*dimX + i]) / (float)(flat[flatSel*dimX*dimY + (j+shiftX)*dimX + (i+shiftY)] - dark[j*dimX + i]);
        if (norm_v != 0.0f) proj_norm_corr[j*dimX + i] = -logf(norm_v);
        else proj_norm_corr[j*dimX + i] = 0.0f;
        }} /*end of ifs*/
   }}
}
