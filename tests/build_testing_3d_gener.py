...
pars = {"input_data": np.float32(Noise3DArray), "radius": 1}  # input grayscale image
...
matplotlib.image.imsave(
    "denoisedGPUshared.png", volume_filteredGPU_shared[:, midval, :]
)
matplotlib.image.imsave(
    "denoised_CPU_GPU_residual.png",
    np.abs(volume_filteredCPU[:, midval, :] - volume_filteredGPU[:, midval, :]),
)
# IPython.embed()
