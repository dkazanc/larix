...
pars = {"input_data": volume, "kernel_size": 3}  # input a grayscale image
...
single_slice_residual = np.sum(
    volume_filteredGPU[:, slice_idx, :] - volume_filteredCPU[:, slice_idx, :]
)
print(
    f"The sum of the CPU and GPU residual for the horizontal slice {slice_idx} of the volume is {single_slice_residual}"
)
...
print(
    f"The sum of the CPU and GPU total residual for the filtered volume is {total_residual}"
)
