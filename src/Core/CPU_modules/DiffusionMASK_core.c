 ...
 * 3. Diffusivity window (half-size of the searching window, e.g. 1)
 * 4. lambda - regularisation parameter (a constant or the same size as the input (1))
...
float DiffusionMASK_CPU_main(float *Input, unsigned char *MASK, float *Output, float *infovector, int DiffusWindow, float *lambdaPar, int lambda_is_arr, float sigmaPar, int iterationsNumb, float tau, int penaltytype, float epsil, int dimX, int dimY, int dimZ)
{
    long i,j,k;
    int counterG;
    ...
    /* Start diffusivity iterations usign MASK */
    for(i=0; i < iterationsNumb; i++) {
      ...
      if (dimZ == 1) {
            ...
            if (sigmaPar == 0.0f) LinearDiff_MASK2D(Input, MASK, Output, Eucl_Vec, DiffusWindow, lambdaPar, lambda_is_arr, tau, (long)(dimX), (long)(dimY)); /* constrained linear diffusion */
            else NonLinearDiff_MASK2D(Input, MASK, Output, Eucl_Vec, DiffusWindow, lambdaPar, lambda_is_arr, sigmaPar2, tau, penaltytype, (long)(dimX), (long)(dimY)); /* constrained nonlinear diffusion */
          }
      else {
...
float LinearDiff_MASK2D(float *Input, unsigned char *MASK, float *Output,  float *Eucl_Vec, int DiffusWindow, float *lambdaPar, int lambda_is_arr, float tau, long dimX, long dimY)
{

long i,j,i1,j1,i_m,j_m,index,indexneighb,counter;
unsigned char class_c, class_n;
float diffVal, lambda_val;

#pragma omp parallel for shared(Input) private(index,i,j,i1,j1,i_m,j_m,counter,diffVal,indexneighb,class_c,class_n,lambda_val)
    for(i=0; i<dimX; i++) {
        for(j=0; j<dimY; j++) {
        index = j*dimX+i; /* current pixel index */
        lambda_val = *(lambdaPar + index* lambda_is_arr);
		...
		counter++;
            ...
            Output[index] += tau*(lambda_val*(diffVal) - (Output[index] - Input[index]));
        }}
	return *Output;
}

/*  MASKED-constrained 2D nonlinear diffusion */
float NonLinearDiff_MASK2D(float *Input, unsigned char *MASK, float *Output, float *Eucl_Vec, int DiffusWindow, float *lambdaPar, int lambda_is_arr, float sigmaPar, float tau, int penaltytype, long dimX, long dimY)
{
	long i,j,i1,j1,i_m,j_m,index,indexneighb,counter;
	unsigned char class_c, class_n;
	float diffVal, funcVal, lambda_val;

#pragma omp parallel for shared(Input) private(index,i,j,i1,j1,i_m,j_m,counter,diffVal,funcVal,indexneighb,class_c,class_n,lambda_val)
    for(i=0; i<dimX; i++) {
        for(j=0; j<dimY; j++) {
        index = j*dimX+i; /* current pixel index */
        lambda_val = *(lambdaPar + index* lambda_is_arr);
		...
		counter++;
           ...
           Output[index] += tau*(lambda_val*(funcVal) - (Output[index] - Input[index]));
		}}
	return *Output;
}
/********************************************************************/
