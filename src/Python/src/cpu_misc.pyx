...
cdef extern int Inpaint_simple_CPU_main(float *Input, unsigned char *Mask, float *Output, unsigned char *M_upd, int iterations, int W_halfsize, int method_type, int dimX, int dimY, int dimZ);
#################################################################
##########################Autocropper ###########################
#################################################################
...
#*********************Euclidian Weighting************************#
#****************************************************************#
def INPAINT_EUCL_WEIGHTED(inputData, maskData, iterationsNumb, windowsize_half, method_type):
    if inputData.ndim == 2:
        if (method_type == 'median'):
            ...
            method_type_int = 0
        return INPAINT_EUC_WEIGHT_2D(inputData, maskData, iterationsNumb, windowsize_half, method_type_int)
    elif inputData.ndim == 3:
...
def INPAINT_EUC_WEIGHT_2D(np.ndarray[np.float32_t, ndim=2, mode="c"] inputData,
    ...
    if (Inpaint_simple_CPU_main(&inputData[0,0], &maskData[0,0], &outputData[0,0], &maskData_upd[0,0], iterationsNumb, windowsize_half, method_type_int, dims[1], dims[0], 1)==0):
        return (outputData, maskData_upd)
    else:
